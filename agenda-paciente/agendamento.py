...
# --- 3. ESTILO VISUAL (ROSE PREMIUM + MODO LIMPO) ---
st.markdown("""
    <style>
        /* Fundo Geral */
        .stApp { background-color: #F0E4E6; }
        
        /* --- ESCONDER BARRA DO STREAMLIT (MODO PROFISSIONAL) --- */
        /* Esconde o menu de hamburguer, botão de deploy, footer e header colorido */
        ...
        /* Ajuste para subir o conteúdo já que tiramos o header */
        .block-container {
...
# --- ÁREA ADMIN PROTEGIDA (O SEGREDO!) ---
with st.sidebar:
    # Tudo escondido dentro deste expander
    with st.expander("🔐 Acesso Restrito"):
        ...
        if senha_admin == "admin123":
            ...
            # Ferramentas Técnicas (Só aparecem com a senha)
            st.markdown("**🛠️ Ferramentas**")
            
            if st.button("📊 Painel Financeiro"):
                ir_para('admin_panel')
            ...
            if st.button("🔌 Testar Conexões"):
                ...
                creds = get_credentials()
                if creds: st.success("Google API OK")
...
if st.session_state.pagina == 'home':
    ...
    st.markdown("""<div class="social-footer"><a href="https://wa.me/5512997997515" target="_blank">💬</a></div>""", unsafe_allow_html=True)
    st.markdown("<div style='margin-top: 20px; text-align: center; color: #7A7A7C; font-size: 12px;'><p>Taubaté/SP | CRO 12345<br>© 2025 Dra. Thais Milene</p></div>", unsafe_allow_html=True)
